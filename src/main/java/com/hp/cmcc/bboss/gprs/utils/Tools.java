...
import com.hp.cmcc.bboss.gprs.pojo.DynamicBean;
...
public class Tools {
	 ...
	 * @return pi中的省代码是否存在
	 */
	public static boolean provinceIdisExist(String pi){
		HashMap<String,Integer> map = new HashMap<String,Integer>();
		map.clear();
		for(int i : PubData.ProvinceId){
			map.put(i+"", i);
		}
	 ...
	 * @return 日期
	 */
	public static java.util.Date strToDate(String d,String fmt){
		SimpleDateFormat sdf = new SimpleDateFormat(fmt);
		java.util.Date date = null;
		try {
			date = sdf.parse(d);
		} catch (ParseException e) {
			L.warn("{} parse error, pls check!",d);
		}
		return date;
	}
